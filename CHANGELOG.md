...
- Fix selecting a password beyond the first one in the list (no more crash on Nano).
- AZERTY layout: third-level characters (`# @ [ \ ] ^ \` { | } ~`) are now typed
  using AltGr (Right Alt) instead of Left Alt, so they map to the correct
  characters on AZERTY hosts.
