...
#define GET_CHAR(char_class, char_idx) ((char*)PIC(screen_keyboard_classes_elements[char_class]))[char_idx]
...
switch(event) {
    ...
    if (GET_CHAR(G_keyboard_ctx.onboarding_step, value) == '\b') {
        ...
        screen_common_keyboard_init(0, G_keyboard_ctx.onboarding_step, (strlen(PP_BUFFER)?5:3), screen_keyboard_class_callback);
        return NULL;
    }
        ...
        if (strlen(PP_BUFFER) >= screen_keyboard_buffer_maxsize /*sizeof(G_keyboard_ctx.words_buffer) - 10*/ /*max pin + pin length*/) {
    ...
    redisplay_current_class:
        ...
        G_keyboard_ctx.onboarding_step = (G_keyboard_ctx.onboarding_step%3) + (strlen(PP_BUFFER)?0:3);
        screen_common_keyboard_init(0, (event==KEYBOARD_ITEM_VALIDATED&& strlen(PP_BUFFER)==0)?0:COMMON_KEYBOARD_INDEX_UNCHANGED, strlen((char*)PIC(screen_keyboard_classes_elements[G_keyboard_ctx.onboarding_step])), screen_keyboard_item_callback);
        return NULL;
    }
    ...
    case KEYBOARD_ITEM_VALIDATED:
        switch (value) {
            case 3:
                ...
                if (strlen(PP_BUFFER)) {
                    PP_BUFFER[strlen(PP_BUFFER)-1] = 0;
                    screen_common_keyboard_init(0, strlen(PP_BUFFER)==0?0:COMMON_KEYBOARD_INDEX_UNCHANGED, (strlen(PP_BUFFER)?5:3), screen_keyboard_class_callback);
                    return NULL;
                }
                break;
            case 4:
            ...
            case 2:
                ...
                screen_common_keyboard_init(0, 0, strlen(&GET_CHAR(G_keyboard_ctx.onboarding_step, 0)), screen_keyboard_item_callback);
                return NULL;
